...
const SendBCH = ({ passLoadingStatus }) => {
    ...
    function handleSendXecError(errorObj, oneToManyFlag) {
        ...
        if (oneToManyFlag) {
            errorNotification(errorObj, message, 'Sending XRG one to many');
        } else {
            errorNotification(errorObj, message, 'Sending XRG');
        }
    }
    ...
    const handleMultiAddressChange = e => {
        ...
        for (let i = 0; i < arrayLength; i++) {
            ...
            if (!validAddress) {
                error = 'Ensure each XRG address is valid';
                setSendBchAddressError(error);
                return setFormData(p => ({
                    ...p,
                    [name]: value,
    ...
    const priceApiError = fiatPrice === null && selectedCurrency !== 'ⵟ';
                ...
                <p>
                    {isOneToManyXECSend ? (
                        <>
                            ...
                            XRG in the following transaction?
                            <br />
                            <br />
                            {formData.address}
                        </>
